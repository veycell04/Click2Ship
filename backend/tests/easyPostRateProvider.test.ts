...
describe('EasyPostRateProvider', () => {
  ...
  it('keeps only USPS rates with retail_rate and never uses rate or list_rate', async () => {
    ...
    expect(await provider.getRates(input)).toEqual([{ providerShipmentId: 'shp_test', providerRateId: 'rate_priority', carrier: 'USPS', serviceCode: 'Priority', serviceName: 'Priority', retailPriceCents: 800, deliveryDays: 2, deliveryDate: null, guaranteed: false }]);
  });
